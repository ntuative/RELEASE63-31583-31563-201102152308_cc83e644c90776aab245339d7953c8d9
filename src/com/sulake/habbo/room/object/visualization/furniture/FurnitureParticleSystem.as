package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1004:Number = 1.0;
      
      private var var_539:BitmapData;
      
      private var var_655:ColorTransform;
      
      private var var_1245:ColorTransform;
      
      private var var_1500:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_724:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_655 = new ColorTransform();
         this.var_655.alphaMultiplier = 1;
         this.var_1245 = new ColorTransform();
         this.var_1500 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_93 = this.var_1003[param1];
         this.var_807 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1501 >= 0)
         {
            this.var_232 = this._visualization.getSprite(this.var_1501);
            if(this.var_232 && this.var_232.asset)
            {
               ...
               if(this.var_81 == null)
               {
                  ...
                  if(this.var_655.alphaMultiplier != 1)
                  {
                     this.var_539 = new BitmapData(this.var_81.width,this.var_81.height,true,4278190080);
                  }
               }
               this.var_2015 = -this.var_232.offsetX;
               this.var_2014 = -this.var_232.offsetY;
            ...
            if(this.var_539)
            {
               this.var_539.fillRect(this.var_539.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_93.y * this.var_1004;
         }
         return 0;
      }
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_807 && this.var_93.hasIgnited)
         {
            this.var_807 = true;
         }
         _loc8_ = this.var_1079 * this.var_1004;
         this.var_93.update();
         if(this.var_807)
         {
            if(this.var_93.roomObjectSpriteId >= 0)
            ...
            {
               this.var_81.draw(this.var_539,this.var_1500,this.var_655,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_93.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2015 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1004;
               _loc4_ = this.var_2014 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1004;
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2013 = parseInt(param1.@size);
         this.var_1501 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1079 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1004 = this.var_2013 / 64;
         this.var_724 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_724 = Math.min(this.var_724,1);
         this.var_655.alphaMultiplier = this.var_724;
