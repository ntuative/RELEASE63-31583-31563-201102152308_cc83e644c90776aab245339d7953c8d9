package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      {
         if(param1 == null || this.var_448 == null)
         {
            return;
         }
         var _loc2_:int = this.var_448.indexOf(param1);
         if(_loc2_ != -1)
         {
            this.var_448.splice(_loc2_,1);
         }
      }
      ...
      {
         var _loc1_:* = null;
         while(this.var_448.length > 0)
         {
            _loc1_ = this.var_448.pop() as IPetDataListener;
            ...
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.HabboCatalogCom_purchaseWidgetCreditsPixelsStub.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1250.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_542.getValue(_loc6_) == null)
               {
                  ...
                  this.var_542.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_448.indexOf(param3) == -1)
                  {
                     this.var_448.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_542.getKeys())
         {
            _loc5_ = this.var_542.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
