package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_705:IFigurePartSet;
      ...
      private const const_1946:int = 16777215;
      
      private const const_325:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1623 = Math.max(this.var_1623,_loc6_.colorLayerIndex);
            }
         }
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_650 != null && !this.var_1622)
            {
               _loc3_ = this.var_650;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_705)
            {
               switch(this.var_705.clubLevel)
               {
                  case HabboClubLevelEnum.const_49:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_60:
                     _loc2_.style = HabboIconType.const_710;
                     break;
                  case HabboClubLevelEnum.const_55:
                     _loc2_.style = HabboIconType.const_879;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_325[_loc6_] + "_" + FigureData.const_134;
               _loc4_ = this.var_401.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_325.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_325[_loc6_] + "_" + FigureData.const_134;
                  _loc4_ = this.var_401.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_563 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1621)
               {
                  ...
                  this.var_1621 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_563.width,this.var_563.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_563.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_563.y;
               if(this.var_1622 && _loc2_.colorLayerIndex > 0)
               {
