package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1901 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1200 = this.var_1901.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1200)
         {
            this.var_1200.procedure = this.onButtonClick;
         }
         this.var_1199 = this.var_1901.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1199)
         {
            this.var_1199.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_265 >= this.var_83.length - 1)
         {
            this.var_1199.disable();
         }
         else
         {
            this.var_1199.enable();
         }
         if(this.var_265 <= 0)
         {
      ...
      {
         ++this.var_265;
         this.var_265 = Math.min(this.var_265,this.var_83.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_265;
         this.var_265 = Math.max(this.var_265,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_88.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_83.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_83 || this.var_83.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_83[this.var_265][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_83[this.var_265][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_83[this.var_265][1]).value;
