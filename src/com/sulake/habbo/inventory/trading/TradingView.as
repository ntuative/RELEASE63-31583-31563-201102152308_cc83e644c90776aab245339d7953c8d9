package com.sulake.habbo.inventory.trading
{
   ...
   public class TradingView implements IInventoryView, IGetImageListener
   {
      ...
      private static const const_951:uint = 4284532064;
      
      private static const const_952:uint = 2577770;
      ...
      public function dispose() : void
      {
         if(!this._disposed)
         {
            ...
            if(this.var_197)
            {
               this.var_197.removeEventListener(TimerEvent.TIMER,this.timerEventHandler);
               this.var_197.stop();
               this.var_197 = null;
            }
      ...
      public function updateUserInterface() : void
      {
         ...
         if(this.var_68)
         {
            ...
            if(_loc1_)
            {
               _loc1_.color = !!this.var_22.method_6 ? uint(const_951) : uint(const_952);
            }
            ...
            if(_loc3_)
            {
               ...
               _loc3_.bitmap.copyPixels(this._assetLibrary.getAssetByName(!!this.var_22.method_6 ? "trading_locked_icon_png" : "trading_unlocked_icon_png").content as BitmapData,_loc3_.bitmap.rect,new Point());
               _loc3_.invalidate();
            }
            _loc1_ = this.var_68.findChildByTag("OTHER_USER_BORDER");
            if(_loc1_)
            {
               _loc1_.color = !!this.var_22.otherUserAccepts ? uint(const_951) : uint(const_952);
            }
         ...
         if(this.var_68)
         {
            ...
            switch(this.var_22.state)
            {
               ...
               case TradingModel.const_168:
                  ...
                  _loc1_.caption = !!this.var_22.method_6 ? "${inventory.trading.modify}" : "${inventory.trading.accept}";
                  this.showInfoMessage("${inventory.trading.info.add}");
                  break;
               case TradingModel.const_341:
                  _loc1_.disable();
      ...
      public function startConfirmCountdown() : void
      {
         ...
         {
            this.var_197 = new Timer(1000,3);
            this.var_197.addEventListener(TimerEvent.TIMER,this.timerEventHandler);
         }
         this.var_197.reset();
         this.var_197.repeatCount = 3;
         this.var_197.start();
      ...
      {
         this._windowManager.registerLocalizationParameter("inventory.trading.countdown","counter",String(3 - this.var_197.currentCount));
         if(this.var_197.currentCount == 3)
         {
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "button_accept":
                  ...
                  {
                     case TradingModel.const_168:
                        if(this.var_22.otherUserItems.length == 0 && !this.var_22.method_6)
                        {
                           this.alertPopup(const_994);
                        }
                        if(this.var_22.method_6)
                        {
      ...
      private function ownThumbEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         else if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER)
         {
            ...
            if(_loc4_.category == FurniCategory.const_1183)
            {
            ...
            this.var_425.updateContent(param2 as IWindowContainer,_loc6_,_loc5_,ItemPopupCtrl.const_570);
            this.var_425.show();
         }
